...
import { ROLE_INFO } from '@/lib/utils'
...
export default function LoginPage() {
  ...
  const quickLogin = (role: Role, dept?: string) => {
    const u = dept
      ? store.users.find(x => x.role === role && x.dept === dept)
      : store.users.find(x => x.role === role)
    if (!u) return
    setEmail(u.email)
    login(u.id)
  }
      ...
      <div className="w-full max-w-[400px] bg-white border border-gray-200 rounded-xl shadow-lg p-9">
        ...
        {!showReset ? (
          <>
            ...
            <div className="flex flex-col gap-2 mt-4 p-3.5 bg-slate-50 rounded-md">
              <div className="text-[11px] text-gray-500 font-medium uppercase tracking-widest">ทดลองในแต่ละบทบาท</div>
              <div className="flex flex-col gap-1.5">
                {store.departments.map(dept => {
                  const roles: Role[] = ['staff', 'officer', 'manager']
                  const available = roles.filter(r => store.users.some(u => u.role === r && u.dept === dept.id))
                  if (available.length === 0) return null
                  return (
                    <div key={dept.id} className="flex items-center gap-2">
                      <span className="text-[10px] text-gray-400 font-mono w-[46px] shrink-0">{dept.short}</span>
                      <div className="flex gap-1 flex-wrap">
                        {available.map(r => (
                          <span
                            key={r}
                            onClick={() => quickLogin(r, dept.id)}
                            className="px-2 py-0.5 rounded-full text-[11px] font-medium cursor-pointer border border-gray-200 bg-white text-gray-700 transition-all hover:border-indigo-500 hover:text-indigo-600 select-none"
                          >
                            {ROLE_INFO[r].th}
                          </span>
                        ))}
                ...
                <div className="flex items-center gap-2 pt-1 border-t border-gray-200 mt-0.5">
                  <span className="text-[10px] text-gray-400 font-mono w-[46px] shrink-0">SYS</span>
                  <span
                    onClick={() => quickLogin('admin')}
                    className="px-2 py-0.5 rounded-full text-[11px] font-medium cursor-pointer border border-rose-200 bg-rose-50 text-rose-700 transition-all hover:bg-rose-100 select-none"
                  >
                    {ROLE_INFO['admin'].th}
                  </span>
                </div>
